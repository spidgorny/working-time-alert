...
export class TrayHandler {
	...
	readonly appId = "eu.nintendo.working-time-alert";

	readonly appName = 'Working Time Alert 2.0';
	...
	autoLauncher: any;
	private appSlogan: string = "Never miss the time to go home";

	constructor(app: any, window: BrowserWindow, store: any, favicon: string, faviconIco: string) {
		this.app = app;
		this.window = window;
		this.store = store;
	...
	async handleTray() {
		const appIcon = new Tray(this.favicon);
...
*/
	...
	createProgramsShortcut() {
		...
		fs.exists(shortcutPath,  (exists) => {
			...
			ws.create(shortcutPath, properties, (err: any) => {
				if (err) throw err;

				// Add the app ID to the shortcut
				ws.addAppId(shortcutPath, this.appId, (err: any) => {
					if (err) throw err;
					// Ready!
				});
			});
